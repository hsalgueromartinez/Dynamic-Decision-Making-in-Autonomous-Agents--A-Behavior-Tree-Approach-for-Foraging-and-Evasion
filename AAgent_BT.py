...
class AAgent:
    ...
    #------------------------------ CUSTOM CODE —------------
    def count_alien_flowers(self):
        """Returns the total number of AlienFlowers in the inventory."""
        total = 0
    ...
    def detect_flower_direction(self):
        sensor_obj_info = self.rc_sensor.sensor_rays[Sensors.RayCastSensor.OBJECT_INFO]
        center_ray = len(sensor_obj_info) // 2
        
        # Collect all rays that hit a flower
        flower_hits = []
        for index, value in enumerate(sensor_obj_info):
            if value and value.get("tag") == "AlienFlower":
                flower_hits.append(index)
                
        # If no flowers are seen, return None
        if not flower_hits:
            return None
            
        # Sort the hits by how close they are to the center ray. This forces the robot to prioritize flowers directly in front of it.
        flower_hits.sort(key=lambda idx: abs(idx - center_ray))
        best_ray = flower_hits[0]
        
        # Steer based on the BEST ray
    ...
    #simiar to the dectect flower helper function, but this time for the Critters, and also considering distances
    def detect_critter_direction(self):
        sensor_obj_info = self.rc_sensor.sensor_rays[Sensors.RayCastSensor.OBJECT_INFO]
        center_ray = len(sensor_obj_info) // 2
        ...
        for index, value in enumerate(sensor_obj_info):
            if value and value.get("tag") == "CritterMantaRay":
                dist = value.get("distance", 999)
                critter_hits.append((index, dist))

        if not critter_hits:
            self.critter_dir = None
            self.critter_dist = 999
            return None

        critter_hits.sort(key=lambda x: (x[1], abs(x[0] - center_ray)))
        best_ray, best_dist = critter_hits[0]
        self.critter_dist = best_dist

        if best_ray < center_ray:
            self.critter_dir = "left"
        elif best_ray > center_ray:
            self.critter_dir = "right"
        else:
            self.critter_dir = "center"

        return self.critter_dir

    def is_critter_close(self, threshold=2.5):
        self.detect_critter_direction()
        return getattr(self, "critter_dist", 999) <= threshold
