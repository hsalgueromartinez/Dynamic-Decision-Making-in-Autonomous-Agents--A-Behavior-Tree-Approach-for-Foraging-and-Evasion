...
class BN_ObstacleAhead(pt.behaviour.Behaviour):
    ...
    def update(self):
        ...
        for index in [1, 2, 3]:
            ...
            if hit and hit.get("tag") in ["Wall", "Rock", "Container"]:
                return pt.common.Status.SUCCESS
...
class BTRoam:
    def __init__(self, aagent):
            ...
            # PPA BRANCH 1: DEPOSIT FLOWERS 
            # memory=False so it constantly checks if it's still at the base
            ...
            # memory=False so it checks leave_seq (is at base?) first on every tick
            ...
            # memory=False so it re-evaluates BN_IsInventoryFull every tick
            ...
            # PPA BRANCH 2: HARVEST FLOWERS
            # memory=False so it constantly checks BN_DetectFlower 
            ...
            # memory=true: This allows the agent to finish a random walk + turn cycle 
            # without restarting the animation/movement constantly.
            ...
            # memory=False so it can interrupt roaming if a flower appears
            ...
            # AUTONOMOUS LOGIC
            # memory=False so it checks inventory full status on EVERY tick
            autonomous_logic = pt.composites.Selector(name="Autonomous Logic", memory=False)
            autonomous_logic.add_children([
                manage_inventory_seq, # Priority 1: Unload if full 
                harvest_selector      # Priority 2: Harvest if empty
            ])

            # ROOT SELECTOR
            self.root = pt.composites.Selector(name="Root Selector", memory=False)
            self.root.add_children([
                frozen_seq,        # Priority 0: Stunned state check [cite: 115]
                autonomous_logic   # Priority 1: Regular logic
            ])
